#!/usr/bin/env python
"""Test script for the migrate actions presentation layer."""
import os
from src.sublime_migration_cli.presentation.factory import create_formatter
from src.sublime_migration_cli.commands.migrate.refactored.actions import migrate_actions_between_instances
...
if __name__ == "__main__":
    ...
    formatter.output_result(result)  # Explicitly output the result
    ...
    if input("\nWould you like to test a real migration? (y/N): ").lower() == 'y':
        include_types = input("Enter action types to include (comma-separated, or leave empty for all): ")
