#!/usr/bin/env python
"""Test script for the migrate exclusions presentation layer."""
import os
from src.sublime_migration_cli.presentation.factory import create_formatter
from src.sublime_migration_cli.commands.migrate.refactored.exclusions import migrate_exclusions_between_instances
...
if __name__ == "__main__":
    ...
    if input("\nWould you like to test a real migration? (y/N): ").lower() == 'y':
        include_system = input("Include system-created exclusions? (y/N): ").lower() == 'y'
        ...
        result = migrate_exclusions_between_instances(
            source_api_key, source_region,
            dest_api_key, dest_region,
            include_system_created=include_system,
            formatter=formatter
        )
        formatter.output_result(result)